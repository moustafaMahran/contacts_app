...
class AuditsController < ApplicationController
  ...
  def show
    ...
    paginated_params = pagination_params(params)
    @audits = Audit.where(auditable_type: permitted_params[:auditable_type], auditable_id: permitted_params[:id]).limit(paginated_params[:page_size]).offset(paginated_params[:offset])
    render json: @audits, status: :ok
  end
